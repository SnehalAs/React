...
const Jobs = () => {
  ...
  const onCHangeEmpTypeList = (value, isChecked) => {
    if (isChecked === true) {
      ...
      setValues({
        ...allValues,
        empTypeList: allValues.empTypeList.filter((each) => each !== value),
      });
    }
  };
    ...
    <div>
      ...
      <div className="jobs-main-cont">
        <div className="filter-section">
          <FilterSection empTypeFunction={onCHangeEmpTypeList} salaryRangeFunction={onChangeSalaryRange} />
        </div>

        <div className="all-jobs-card-cont">
          <input
            onKeyUp={onChangeUserSearch}
            type="search"
            className="form-control w-50 mb-3 ml-5"
          />
          <ul>
